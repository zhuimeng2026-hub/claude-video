...
- `skills/watch/scripts/watch_to_remotion.py` / `watch_to_remotion_smart.py` — `/watch` → Remotion 4.x converters (deterministic + LLM-driven variants). **Re-enabled 2026-08-23** (were temporarily parked as `.py_tmp` in commit `1cac74b`, then accidentally deleted, now restored as `.py` so the canonical filenames work). See `docs/todo.md` §2.6.1 for the adapter-refactor work that's still pending — the scripts are importable again but **the §2.6 "v2+ must route through OpenMontage" hard constraint still applies**, so don't invoke them from the `/watch` MCP server's main flow.
