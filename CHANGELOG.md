...
> **Status (2026-08-23, updated)**: **Re-enabled.** The `.py_tmp` files added in v0.3.0 were accidentally deleted in commit `0538676` (Phase 2.x side-effect). On 2026-08-23 the user restored them and renamed back to `.py`. The scripts are importable again. The §2.6.1 adapter refactor / env guard / guard test remain open as **regression-prevention work** — see `docs/todo.md` §2.6.1 for the four pending items. The original 2026-08-23 status block (disabled with `.py_tmp` park) is preserved below for the historical record.

> **Status (post-0.3.0, 2026-08-23, superseded)**: both scripts above were **temporarily disabled** in commit `1cac74b` — the `.py` files were renamed to `.py_tmp` (i.e. `watch_to_remotion.py_tmp`, `watch_to_remotion_smart.py_tmp`) so Python's import system would ignore them. They were kept on disk for re-enable once the adapter refactor in `docs/todo.md` §2.6 was complete. Nothing in `SKILL.md` or `watch.py` invoked them while disabled. **Superseded** by the updated status above — the files were accidentally deleted in commit `0538676` and restored on 2026-08-23.
